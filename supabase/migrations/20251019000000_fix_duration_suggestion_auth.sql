-- ============================================================
-- FIX: Duration Suggestion RPC - Auth Context Issue
-- ============================================================
-- Problem: auth.uid() doesn't work correctly when called from API route
-- Solution: Accept user_id as explicit parameter instead
-- ============================================================

-- Drop old function first (must specify exact signature)
DROP FUNCTION IF EXISTS get_duration_suggestion(TEXT, TEXT, FLOAT, FLOAT);

CREATE OR REPLACE FUNCTION get_duration_suggestion(
  p_title TEXT,
  p_content TEXT,
  p_title_threshold FLOAT DEFAULT 0.60,
  p_content_threshold FLOAT DEFAULT 0.40,
  p_user_id UUID DEFAULT NULL  -- NEW: Accept user_id as parameter
)
RETURNS TABLE(
  avg_duration FLOAT,
  median FLOAT,
...
BEGIN
  ...
  target_user_id := COALESCE(p_user_id, auth.uid()::uuid);

  IF target_user_id IS NULL THEN
    RAISE EXCEPTION 'user_id is required (either via parameter or auth.uid())';
  END IF;
  ...
  WITH similar_slides AS (
    ...
    WHERE similarity(sf.title_normalized, normalized_title) > p_title_threshold
      ...
      AND sf.user_id = target_user_id  -- Use explicit user_id parameter
  ),
...
GRANT EXECUTE ON FUNCTION get_duration_suggestion(TEXT, TEXT, FLOAT, FLOAT, UUID) TO authenticated;

-- Add function comment
COMMENT ON FUNCTION get_duration_suggestion IS 'Get duration suggestion for a slide based on similarity matching with IQR outlier filtering. Accepts optional user_id parameter.';
